...
# Shows 5 questions one at a time, each with 4 options (A, B, C, D)
# Accepts the player's answer (case-insensitive)
# Tells the player immediately if they were right or wrong
# Keeps a running score
# Shows a final result with score, percentage, and a message based on performance
# Allows the player to quit early by typing "quit"
...
def run_quiz():
    ...
    for i, question_data in enumerate(questions, start=1):
        ...
        if check_answer(user_answer, question_data["answer"]):
            ...
            print(f"❌ Wrong! The correct answer was {correct_option}) {correct_answer_text}\n")

    show_results(score, total_questions, quit_early)
